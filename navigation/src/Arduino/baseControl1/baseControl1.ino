...
ros::Publisher rpm_value1("rpm_value1", &str_msg);
...
PWM = msg.x;//receives both +ve and negative values acc to motion needed )

}


//Subscriber
ros::Subscriber<geometry_msgs::Vector3> sub("keyboard_message1", &rotate);
...
void setup() {
 ...
 nh.initNode();
 nh.subscribe(sub);
 nh.advertise(rpm_value1); //Publish
